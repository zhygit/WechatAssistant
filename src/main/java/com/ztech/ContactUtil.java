 ...
 *
 */
public class ContactUtil implements ILoginService {
	private static Logger LOG = LoggerFactory.getLogger(ContactUtil.class);
	...
	public boolean webWxInit() {
		...
		try {
			...
			LOG.info("========================================================");
			LOG.info("Init User : "+ user.toJSONString());
			...
			public void run() {
				while (core.isAlive()) {
					try {
						...
						LOG.info("syncCheck : "+JSONObject.toJSONString(resultMap));
...
//										LOG.info("+++++++   msgList   277+++++++");
//										LOG.info("\n\r\n\r\n\r\n\r");
//										LOG.info(msgList.toJSONString());
//										LOG.info("\n\r\n\r\n\r\n\r");
//										LOG.info("++++++++++++++");
...
//			zhy 2019-03-25		把自己也保留在联系人列表中，方面msg的统一log
//           可以统一使用Wechat.getNickNameByUserName方法 处理
...
//		paramMap.put("Count", core.getGroupIdList().size());
		...
		try {
			...
			LOG.info(contactList.toJSONString());
			LOG.info("====================================================");
			for (int i = 0; i < 3; i++) { // 群好友
			...
			LOG.info("===========  GroupID List===========================");
			LOG.info("groupIDlist : "+ core.getGroupIdList().toString());
			LOG.info("====================================================");

			LOG.info("===========  GroupNickName List===========================");
			LOG.info("groupIDlist : "+ core.getGroupNickNameList().toString());
			LOG.info("====================================================");

			LOG.info("===========  GroupMember List===========================");
			LOG.info("groupIDlist : "+ core.getGroupMemeberMap().toString());
			LOG.info("====================================================");
	...
	public void updateGroupContact(List<String> groupIDlist) {
		String url = String.format(URLEnum.WEB_WX_BATCH_GET_CONTACT.getUrl(),
				core.getLoginInfo().get(StorageLoginInfoEnum.url.getKey()), new Date().getTime(),
				core.getLoginInfo().get(StorageLoginInfoEnum.pass_ticket.getKey()));
		Map<String, Object> paramMap = core.getParamMap();
		paramMap.put("Count", core.getGroupIdList().size());
		paramMap.put("Count", groupIDlist.size());
		List<Map<String, String>> list = new ArrayList<Map<String, String>>();
		for (int i = 0; i<groupIDlist.size(); i++) {
			HashMap<String, String> map = new HashMap<String, String>();
			map.put("UserName", groupIDlist.get(i));
			map.put("EncryChatRoomId", "");
			list.add(map);
		}
		paramMap.put("List", list);
